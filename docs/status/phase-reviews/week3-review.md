...
was modified (enforced byte-level by `scripts/standard_ppo/14_gate_week3.py`).
...
- `scripts/standard_ppo/10_train_ppo.py --config configs/week3-ppo.yaml --seed 42`: SB3 2.3.2 PPO,
...
- Frozen selection rule (lowest dev `cost_total`; tie-break lower
  `discomfort_proportion`) executed by `scripts/standard_ppo/11_evaluate_checkpoints.py`;
...
- Final-window (0–719) evaluations of each seed's selected checkpoint via
  `scripts/standard_ppo/12_evaluate_final_window.py`, artifacts under `results/runs/ppo/seed<seed>/final/` in
...
- `./.venv/bin/python scripts/standard_ppo/14_gate_week3.py` exits 0 (also via
