...
All artifacts are regenerable via `scripts/10_run_baselines.py` and `scripts/11_compare_baselines.py`; `scripts/12_gate_week2.py` is the phase gate. The comparison is single-seed heuristic evidence under one tariff profile and must not be reported as a savings claim.
