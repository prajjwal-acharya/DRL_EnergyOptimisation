...
All artifacts are regenerable via `scripts/cmdp_baselines/06_run_baselines.py` and `scripts/cmdp_baselines/07_compare_baselines.py`; `scripts/cmdp_baselines/08_gate_week2.py` is the phase gate. The comparison is single-seed heuristic evidence under one tariff profile and must not be reported as a savings claim.
