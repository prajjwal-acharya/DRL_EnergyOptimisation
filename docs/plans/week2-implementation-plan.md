...
- `scripts/foundation/05_gate_week1.py`, `tests/test_environment.py` (2 tests, must keep passing).
...
`scripts/cmdp_baselines/07_compare_baselines.py` — reads all run dirs for a window and writes
...
5. `scripts/cmdp_baselines/07_compare_baselines.py` for both windows; record `Ē_B0`, `P_ref`, `P_grid,max`
   values into `docs/reference/cmdp-spec.md` (fill the placeholders).

### D3. Verification

`scripts/cmdp_baselines/08_gate_week2.py` (mirror the structure of `scripts/foundation/05_gate_week1.py`). It must check,
...
**Acceptance gate D:** `scripts/cmdp_baselines/08_gate_week2.py` exits 0; repo green; all changes committed.
...
3. `results/tables/baseline_comparison.csv` + ≥4 comparison figures exist and are regenerable
   from `scripts/cmdp_baselines/07_compare_baselines.py`.
4. `./.venv/bin/python -m pytest -q` passes (week-1 tests included).
5. `./.venv/bin/python scripts/cmdp_baselines/08_gate_week2.py` exits 0.
